...
public class SyntaxTreeRuleIndividual extends SyntaxTreeIndividual implements IClassifierIndividual 
{
	...
	{
		if (other instanceof SyntaxTreeRuleIndividual) {
			ExprTree otherRule = ((SyntaxTreeRuleIndividual) other).getPhenotype().getAntecedent();
			ExprTree thisRule  = getPhenotype().getAntecedent();
			
			if(thisRule.size() != otherRule.size())
				return false;
			else
				for(int i = thisRule.size()-1; i >= 0; i--)
					if(!thisRule.getBlock(i).equals(otherRule.getBlock(i)))
							return false;
				return true;
		}
		else
